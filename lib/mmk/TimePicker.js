...
export class TimePicker {
  constructor(options = {}) {
    ...
    this.widgets = [];
    this.ownContainer = false; // Track if we created our own container
    this.confirmed = false; // Guard against multiple confirm calls
  }
  ...
  render() {
    ...
    // Create fixed container if not provided (like PriorityPicker does)
    if (!this.container) {
  ...
  confirm() {
    ...
    if (this.confirmed) {
      console.log('TimePicker: confirm already called, ignoring');
      return;
    }
    this.confirmed = true;
