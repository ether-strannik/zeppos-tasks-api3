...
const logger = getLogger('device-message')
...
export class MessageBuilder extends EventBus {
  constructor({ appId = 0, appDevicePort = 20, appSidePort = 0 } = {
    ...
    super()
    this.isDevice = true  // Always device in API 3.0 context
    this.isSide = false

    this.appId = appId
    this.appDevicePort = appDevicePort
    this.appSidePort = appSidePort
    this.sendMsg = this.sendBin.bind(this)
    this.chunkSize = 2000
    this.tempBuf = null
  ...
  connect(cb) {
    ...
    // API 3.0: Use @zos/ble instead of global hmBle
    ble.createConnect((index, data, size) => {
      logger.log('createConnect received size=' + size)
  ...
  sendBin(buf) {
    // API 3.0: Use @zos/ble instead of global hmBle
    logger.log('sendBin size=' + buf.byteLength)
    ble.send(buf.buffer, buf.byteLength)
  }
  ...
  sendDataWithSession({ traceId, spanId, seqId, payload, type, opCode, totalLength }, { messageType }, ) {
    ...
    let data = this.buildData(payloadBin, { type: messageType })
    this.sendMsg(data)
  }
  ...
  readPayload(arrayBuf) {
    ...
    // Skip timestamps and extras (40 bytes)
    offset += 40
